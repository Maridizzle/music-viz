...
export class Bubbles implements Preset {
  ...
  update(frame: AudioFrame, params: PresetParams, dt: number, t: number): void {
    ...
    const pop = this.beatPulse * beatPop * 0.35;

    for (let i = 0; i < this.count; i++) {
      let yi = this.y[i]! + dt * riseSpeed * (0.4 + frame.level * 1.4) * this.speed[i]!;
      if (yi > TOP) {
        this.spawn(i);
        yi = -TOP;
      }
      ...
      const x = this.baseX[i]! + Math.sin(ph) * wobble * 0.15;
      const z = this.baseZ[i]! + Math.cos(ph * 0.8) * wobble * 0.15;
      const radius = size * (0.5 + frame.level * 1.1 + this.sizeVar[i]! * 0.9) + pop;
    ...
    this.material.emissiveIntensity = 0.4 + frame.level * 0.8 + this.beatPulse * 0.8;
  }
